...
(Большинство действий в последующих функциях будут анологичными, поэтому я не буду их комментировать)
 ...
 def finall():
    ...
    cur.execute(f"""
        UPDATE pings
        SET
            work_end = {int(time.time())},       # Здесь производится проверка всех уствойств в базе        
            work_time = {int(time.time())} - (   # для них задаётся конечное время работы
                SELECT work_start                # и вычисляется общее время работы
                FROM pings AS p
                WHERE
